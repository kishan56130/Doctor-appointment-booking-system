...
const AdminAppointments = () => {
  const { appointments, getAllAppointments, cancelAppointment, loading } = useContext(AdminContext);
    ...
    <div className='flex flex-col gap-6 animate-fadeIn w-full'>
      ...
      <div>
        <h2 className='text-2xl font-bold text-slate-800 dark:text-white flex items-center gap-2'>
          <CalendarDays size={24} className='text-primary' /> All Appointments Log
        </h2>
        <p className='text-xs text-slate-500 dark:text-slate-400 mt-1'>
          Track scheduled consults across the entire platform and handle administrative cancellations if needed.
        </p>
      </div>

      {loading && appointments.length === 0 ? (
        <div className='flex justify-center items-center py-20 text-slate-500'>
          <p>Retrieving booking logs...</p>
        </div>
        ...
        <div className='bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 md:p-8 shadow-sm transition-colors duration-200 overflow-x-auto w-full'>
          <table className='w-full text-left text-sm border-collapse min-w-[700px]'>
            ...
            <tbody className='divide-y divide-slate-105 dark:divide-slate-800/50'>
              {appointments.map((item, index) => (
                <tr key={index} className='hover:bg-slate-50/50 dark:hover:bg-slate-850/30 transition-colors'>
                  <td className='py-4 pl-2'>
                    ...
                    <p className='text-[10px] text-slate-400 mt-0.5'>{item.patientData?.email}</p>
                  </td>
                  <td className='py-4'>
                    <div className='flex items-center gap-2'>
                      <img className='w-7 h-7 rounded-full object-cover bg-slate-100 dark:bg-slate-800' src={item.doctorData?.image} alt={item.doctorData?.name} />
                      <div>
                        <p className='font-bold text-xs text-slate-700 dark:text-slate-300'>{item.doctorData?.name}</p>
                        <p className='text-[9px] text-slate-400 font-medium tracking-wide uppercase'>{item.doctorData?.specialty}</p>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className='py-4 text-right pr-2'>
                    {item.status === 'cancelled' ? (
                      ...
                      <div className='flex items-center justify-end gap-2.5'>
                        <span className='inline-flex items-center gap-1 text-blue-600 text-xs font-semibold px-2.5 py-1 bg-blue-50 dark:bg-blue-950/20 rounded-lg border border-blue-100 dark:border-blue-900/30'>
        ...
        <div className='flex flex-col items-center justify-center py-20 text-slate-500 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-8 w-full shadow-sm'>
          ...
          <p className='text-sm max-w-xs text-center mt-1'>There are currently no scheduled appointments in the system logs.</p>
        </div>
      )}
    </div>
  );
